/*
 *	code to be added to diplay function:
 *
 	glGetFloatv(GL_MODELVIEW_MATRIX, mNow);
 	arc->getMatrix(mNow);
 	glLoadMatrixf(mNow);
 *
 *	code to be added to mouse click function:
 *	
 	arc->startBall(event->x(),event->y(),this->width(),this->height());
 *
 *	code to be added to mouse drag:
 *
 ...
 *		needs to be modified based on what click coordinate system is bein used
 */
 ...
 */
void arcBall::getMatrix(float * matrix){
	currQuat.toMatrix(matrix);
}
